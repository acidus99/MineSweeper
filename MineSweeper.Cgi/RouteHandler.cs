...
namespace MineSweeper.Cgi
{
	public static class RouteHandler
	{
		...
		public static void StartGame(CgiWrapper cgi)
        {
            ...
            if(!cgi.HasQuery)
            {
                cgi.Input("Enters rows, columns, and mines for game. For example: '8,12,15' for board with 8 rows, 12 columns, and 15 mines");
                return;
            }
            ...
            try
            {
                GameState state = GameEngine.CreateNewGame(parsedInts[0], parsedInts[1], parsedInts[2]);
