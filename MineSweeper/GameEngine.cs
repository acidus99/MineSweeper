...
public class GameEngine
{
    ...
    public void UpdateState(Move move)
    {
        ...
        if (!State.Board.IsInBounds(move.Row, move.Column)) return;
        ...
        if (move.IsClick)
        {
            if (State.Board.IsShown(move.Row, move.Column))
            {
                ...
                //so do "chording"
                if (State.Board.HasAdjacentMines(move.Row, move.Column)) RevealUnflaggedTiles(move.Row, move.Column);
                return;
            }
    ...
    private void RevealTile(int row, int column)
    {
        ...
        if (State.Board.IsMine(row, column)) return;

        var count = State.Board.AdjacentMineCount(row, column);
    ...
    private Move ParseMove(string move)
    {
        ...
        if (move.ToLower() == "xyzzy") return new Move { IsCheat = true };

        //if we don't have 2 letters, its bad
        if (move.Length != 2 || !char.IsLetter(move[0]) || !char.IsLetter(move[1])) return null;
    ...
    {
        //do validation
        if (rows < 2 || rows > 26) throw new ArgumentException("Rows must be between 2 and 26.");
        if (columns < 2 || columns > 26) throw new ArgumentException("Columns must be between 2 and 26.");
        if (mines < 1 || mines > 255 || mines >= rows * columns)
            throw new ArgumentException(
                "Mines must be between 1 and 256, and cannot be more than the size of the board.");
